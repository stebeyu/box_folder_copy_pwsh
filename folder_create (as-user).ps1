...
ForEach ($Folder in $folderCreateInput) {
    ...
    if($errorReceived){
        ...
        if ($errorMessage.Contains('operation_blocked_temporary')){
                ...
                $retryResult = Invoke-Expression "box folders:copy $FolderID $FolderParent --as-user $FolderAsUser --name $FolderName --json" -ErrorVariable retryErrorMessage 2>$null
                ...
                if($retryErrorMessage){
                    ...
                    Add-content -path $OutputFile -value "$row,$FolderID,$FolderParent,$FolderAsUser,$FolderName,failed,$retryErrorMessage"
                    }
                ...
                elseif ($retryResult){
                    ...
                    Add-content -path $OutputFile -value "$row,$newFolderIDRetry,$newFolderParentRetry,$FolderAsUser,$newFolderNameRetry,success,"
                }
                else{
                    Write-Host "Row $row error received: Copy failed on retry for item $FolderID - please review logs and retry later `n" -Fore Red
                    Add-content -path $OutputFile -value "$row,$FolderID,$FolderParent,$FolderAsUser,$FolderName,failed,"
                }
            }
        ...
        elseif($errorMessage.Contains('item_name_in_use')){
            ...
            Add-content -path $OutputFile -value "$row,$FolderID,$FolderParent,$FolderAsUser,$FolderName,failed,$errorReceived"
        }
    ...
    elseif ($createFolder){
        ...
        Add-content -path $OutputFile -value "$row,$newFolderID,$newFolderParent,$FolderAsUser,$newFolderName,success,"
    }
